...
**Сразу в первом сообщении попроси API ключ Polza.ai!**

### Первое сообщение пользователю:

> Для подключения ChatGPT мне нужен API ключ от Polza.ai.
>
> **Если у вас уже есть ключ** — пришлите его мне.
>
> **Если нет:**
> 1. Зарегистрируйтесь на [polza.ai](https://polza.ai)
> 2. Перейдите в [Ключи API](https://polza.ai/dashboard/api-keys)
> 3. Нажмите "Создать API ключ"
> 4. Скопируйте ключ (начинается с `ak_`)
> 5. Пополните баланс в [Платежи](https://polza.ai/dashboard/billing) (от 100₽)
>
> Пришлите мне ваш API ключ, и я всё настрою!

### Когда пользователь прислал ключ:

**ОБЯЗАТЕЛЬНО вызови тулу `put_secret`:**

```python
put_secret("POLZA_AI_API_KEY", "<ключ от пользователя>")
```

Только после этого переходи к деплою функции и фронтенду.

---

## Чеклист установки

### Шаг 1: Получить API ключ и сохранить через put_secret (см. выше)
